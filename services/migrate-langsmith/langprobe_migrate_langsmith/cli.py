"""LangSmith export → langprobe ingest CLI.
...
try:
    ...
    raise SystemExit("langprobe-migrate-langsmith requires httpx; pip install httpx") from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="Migrate LangSmith export rows to a langprobe ingest host.",
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    parser.add_argument(
        "--endpoint",
        default=os.environ.get("LANGPROBE_INGEST_URL")
        or os.environ.get("LANGSMITH_ENDPOINT")
        or "http://localhost:7080",
        help="langprobe ingest base URL (default: env LANGPROBE_INGEST_URL).",
    )
    parser.add_argument(
        "--api-key",
        default=os.environ.get("LANGPROBE_INGEST_KEY") or os.environ.get("LANGSMITH_API_KEY"),
        help="Bearer token for the ingest host (default: env LANGPROBE_INGEST_KEY).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    if not args.dry_run and not args.api_key:
        print(
            "warning: no --api-key or LANGPROBE_INGEST_KEY set; the ingest host "
