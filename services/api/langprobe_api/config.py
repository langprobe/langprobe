...
class Settings:
    ...
    session_cookie_name: str = "langprobe_session"
...
def load() -> Settings:
    postgres_dsn = os.environ.get("LANGPROBE_PG_DSN")
    session_secret = os.environ.get("LANGPROBE_SESSION_SECRET")
    if not postgres_dsn:
        raise RuntimeError("LANGPROBE_PG_DSN is required")
    if not session_secret or len(session_secret) < 32:
        raise RuntimeError("LANGPROBE_SESSION_SECRET is required and must be >= 32 chars")
    return Settings(
        postgres_dsn=postgres_dsn,
        session_secret=session_secret,
        clickhouse_url=os.environ.get("LANGPROBE_CLICKHOUSE_URL"),
        redis_url=os.environ.get("LANGPROBE_REDIS_URL"),
        session_cookie_name=os.environ.get("LANGPROBE_SESSION_COOKIE", "langprobe_session"),
        session_max_age_seconds=int(
            os.environ.get("LANGPROBE_SESSION_MAX_AGE_SECONDS", str(60 * 60 * 24 * 7))
        ),
        bind_host=os.environ.get("LANGPROBE_BIND_HOST", "0.0.0.0"),
        bind_port=int(os.environ.get("LANGPROBE_API_BIND_PORT", "7081")),
        log_level=os.environ.get("LANGPROBE_LOG_LEVEL", "INFO"),
        cors_allow_origin=os.environ.get("LANGPROBE_CORS_ALLOW_ORIGIN", "http://localhost:7090"),
        ...
        web_base_url=os.environ.get("LANGPROBE_WEB_BASE_URL", "http://localhost:7090"),
